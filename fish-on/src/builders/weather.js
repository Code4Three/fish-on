...
function processWeatherResponse(weather) {
  ...
  for (let i = 0; i < (weather.hourly?.time ?? []).length; i += 1) {
    ...
    const entry = {
      ...
      windDirection: getWindDirectionLabel(weather.hourly.wind_direction_10m?.[i]),
  ...
  const days = (weather.daily?.time ?? []).map((dateString, index) => {
    ...
    const dailyHours = Array.from({ length: 24 }, (_, hourIndex) => {
      ...
      const candidateHours = Array.from(hourlyMap.keys()).map(Number).sort((a, b) => a - b);
      const closest = candidateHours.reduce((best, candidate) => {
        const currentDistance = Math.abs(candidate - hourIndex);
        const bestDistance = best == null ? Number.POSITIVE_INFINITY : Math.abs(best - hourIndex);
        return currentDistance < bestDistance ? candidate : best;
      }, null);

      return closest == null ? null : hourlyMap.get(String(closest).padStart(2, "0"));
    }).filter(Boolean);
    ...
    const windDirection = getWindDirectionLabel(weather.daily.wind_direction_10m_dominant?.[index]);
    const pressureValues = dailyHours.map(hour => hour.pressure).filter(v => v != null);
    const pressureMin = pressureValues.length ? Math.min(...pressureValues) : null;
    const pressureMax = pressureValues.length ? Math.max(...pressureValues) : null;
    const cloudValues = dailyHours.map(hour => hour.cloudCover).filter(v => v != null);
    const cloudCover = cloudValues.length ? Math.round(cloudValues.reduce((sum, value) => sum + value, 0) / cloudValues.length) : null;
    const summary = getWeatherLabel(weather.daily.weather_code?.[index]);

    return {
      date,
      summary,
      tempRange: tempMin != null && tempMax != null ? [tempMin, tempMax] : [null, null],
      windRange: windMax != null ? [Math.max(0, windMax - 6), windMax] : [null, null],
      windBaseline: windMax != null ? `${windMax} km/h ${windDirection ?? ""}`.trim() : null,
      cloudCover,
      cloudBaseline: cloudCover,
      pressureRange: pressureMin != null && pressureMax != null ? [pressureMin, pressureMax] : [null, null],
      hours: dailyHours
    };
  });
